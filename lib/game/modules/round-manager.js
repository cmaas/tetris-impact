...
.defines(function() {
    ...
    RoundManager = ig.Class.extend({
        ...
        handleVictory: function() {
            this.font.draw('YOU WON', 96, 64, ig.Font.ALIGN.CENTER);
            var blink = mmod(this.gameOverBlinkTimer.delta(), 1.5); // speed of the non-showing state (1.1 = hidden only short)
            if (blink <= 1) {
                this.font.draw("PLEASE\nPLAY\nAGAIN\n\n(PRESS X)", ig.global.TILESIZE * 6, ig.global.TILESIZE * 8, ig.Font.ALIGN.CENTER);
            }
        ...
        generateNoiseBlocks: function() {
            ...
            for (var y = ig.global.FIELD_ROWS - (ig.game.levelSelector.getSelectedLevel() - 1) * 3; y < ig.global.FIELD_ROWS; y++) {
                var blocksInRow = 0;
         ...
         */
        startRound: function( mode, startStage ) {
            this.mode = mode;
            if (this.mode == 'B') {
